...
class Extractor3D(object):
    ...
    def __init__(self, model, points_batch_size=100000,
                 ...
                 with_normals=False, padding=2.4,
    ...
    def generate_mesh(self, data=None, return_stats=True, mask_loader=None, clip=False):
    ...
    def generate_from_latent(self, c=None, stats_dict={}, data=None,
        ...
        if self.upsampling_steps == 0:
            ...
            while points.shape[0] != 0:
                ...
                if self.shift is not None:
                    pointsf += torch.tensor(self.shift).to(pointsf.device)
        ...
        if kwargs['clip']:
            pointsf2 = box_size * make_3d_grid((-0.5,)*3, (0.5,)*3, (value_grid.shape[0],)*3).reshape(*value_grid.shape,3)
            value_grid[pointsf2[...,-1]<-1]=-30.0

        pointsf2 = box_size * make_3d_grid((-0.5,)*3, (0.5,)*3, (value_grid.shape[0],)*3).reshape(*value_grid.shape,3)
        if self.shift is not None:
            pointsf2 += torch.tensor(self.shift).to(pointsf2.device)
            self.wall += self.shift[0]
            self.desk += self.shift[2]
        vsuf = F.normalize(pointsf2.to(self.camloc.device) - self.camloc, dim=-1).view(*value_grid.shape,1,3)
        pdot = (vsuf*self.n_plane).sum(-1)
        mask_out = (pdot<0).any(-1).cpu()
        value_grid[mask_out]=-30.0
        value_grid[pointsf2[...,0]<self.wall]=-30.0
        value_grid[pointsf2[...,-1]<self.desk]=-30.0
